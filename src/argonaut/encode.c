...
SHARC_FORCE_INLINE SHARC_KERNEL_ENCODE_STATE sharc_argonaut_encode_checkState(sharc_byte_buffer *restrict out, sharc_argonaut_encode_state *restrict state) {
    ...
    switch (state->shift) {
        case 64:
            if ((returnState = sharc_argonaut_encode_prepareNewBlock(out, state, SHARC_HASH_ENCODE_MINIMUM_OUTPUT_LOOKAHEAD))) {
...
SHARC_FORCE_INLINE void one_round(const uint8_t* restrict letter, uint64_t * restrict hash, sharc_argonaut_dictionary* restrict dictionary) {
    *hash = *hash ^ *letter;
    *hash = *hash * SHARC_HASH_PRIME;
    sharc_dictionary_primary_entry *primaryHit = &dictionary->prim[*letter];
    primaryHit->durability++;
    if (primaryHit->isRanked) {
        //offset += 6;//(0x80 - primHit->ranking) / 32 + 3;
        if (primaryHit->ranking) {
            sharc_dictionary_primary_entry *pointerPreceding1 = primaryHit->preceding;// dictionary->ranking.topPrim[preceding];
            if (pointerPreceding1->durability < primaryHit->durability) {
                primaryHit->ranking = pointerPreceding1->ranking;
                pointerPreceding1->ranking += 1;
                primaryHit->preceding = pointerPreceding1->preceding;// dictionary->ranking.topPrim[preceding - 1];
                pointerPreceding1->preceding = primaryHit;
                if(primaryHit == dictionary->ranking.lastPrimRanked)
        ...
        if (pointerPreceding2->durability < primaryHit->durability) {
            primaryHit->ranking = pointerPreceding2->ranking;// preceding;
            primaryHit->isRanked = true;
            primaryHit->preceding = pointerPreceding2->preceding;// dictionary->ranking.topPrim[preceding - 1];
            pointerPreceding2->isRanked = false;
        }
    }
}

SHARC_FORCE_INLINE uint8_t byteSearch(uint64_t * restrict hash, word * restrict aword, const uint32_t xorMask, const uint32_t separator, sharc_argonaut_dictionary* restrict dictionary) {
    uint8_t letter;
    *hash = SHARC_HASH_OFFSET_BASIS ^ xorMask;
    if ((letter = aword->letters[0]) ^ separator) {
        one_round(&letter, hash, dictionary);
...
SHARC_FORCE_INLINE void sharc_argonaut_encode_kernel(sharc_byte_buffer *restrict in, sharc_byte_buffer *restrict out, uint64_t *restrict hash, const uint32_t xorMask, sharc_argonaut_dictionary *restrict dictionary, sharc_argonaut_encode_state *restrict state, const uint8_t separator) {
    state->count[7]++;
    ...
    aword.length = byteSearch(hash, &aword, xorMask, separator, dictionary);
    ...
    //if (!(state->count[7] & 0x0)) {
        ...
        aword.compressedLength = aword.length;//(uint8_t) ((offset >> 3) + 1);// aword.length;//
    //} else
    ...
    uint32_t shash = (uint32_t) ((((*hash >> bits) ^ *hash) & ((1 << bits) - 1)));
    ...
    if (aword.as_uint64_t ^ secondaryHit->aword.as_uint64_t /*!compare(&aword, &secondaryHit->aword)*/) {
        ...
        *(uint64_t *) (out->pointer + out->position) = 0;//0x80000000 | (chunk >> 2);
        ...
        if (secondaryHit->ranked) {
            ...
            const uint16_t preceding = (1 << 4) + (1 << 8) - 1;
...
SHARC_FORCE_INLINE void sharc_argonaut_encode_process_span(uint64_t *chunk, sharc_byte_buffer *restrict in, sharc_byte_buffer *restrict out, uint64_t *restrict hash, const uint32_t xorMask, sharc_argonaut_dictionary *restrict dictionary, sharc_argonaut_encode_state *restrict state, const uint8_t separator) {
...
SHARC_FORCE_INLINE SHARC_KERNEL_ENCODE_STATE sharc_argonaut_encode_init(sharc_argonaut_encode_state *state, sharc_argonaut_dictionary *restrict dictionary) {
    ...
    for (uint16_t i = 0; i < PRIM_RANKS; i++) {
        ...
        if (i)
            dictionary->prim[i].preceding = &dictionary->prim[i - 1];
    }
...
SHARC_FORCE_INLINE SHARC_KERNEL_ENCODE_STATE sharc_argonaut_encode_process(sharc_byte_buffer *restrict in, sharc_byte_buffer *restrict out, const uint32_t xorMask, sharc_argonaut_dictionary *restrict dictionary, sharc_argonaut_encode_state *restrict state, const sharc_bool flush) {
    ...
    switch (state->process) {
        ...
        case SHARC_ARGONAUT_ENCODE_PROCESS_DATA:
            ...
            while (true) {
                sharc_argonaut_encode_process_span(&chunk, in, out, &hash, xorMask, dictionary, state, separator);
